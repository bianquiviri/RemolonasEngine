...
class DashboardController extends Controller
{
    ...
    public function customer(Request $request)
    {
        ...
        return response()->json([
            ...
            'next_delivery' => Order::whereHas('subscription', fn($q) => $q->where('user_id', $userId))
    ...
    {
        // En una implementación real, el operador tendría un store_id asignado en su perfil
        $storeId = Store::first()->id; 

        return response()->json([
            'pending_orders' => Order::where('store_id', $storeId)->where('status', 'pending')->count(),
            'picking_now' => Order::where('store_id', $storeId)->where('status', 'picking')->count(),
            'ready_today' => Order::where('store_id', $storeId)->where('status', 'ready_for_dispatch')->count(),
            'recent_orders' => Order::where('store_id', $storeId)->orderBy('created_at', 'desc')->take(5)->get(),
        ]);
    }
